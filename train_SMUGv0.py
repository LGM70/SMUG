...
optimG = torch.optim.Adam(netG.parameters(), lr=opt.lr)
# scheduler = torch.optim.lr_scheduler.MultiStepLR(optimG, milestones=[15,30,50], gamma=0.5)
...
def Recon(cg_iter, smap, mask, input, label, smoothing=False, num_sample=10, epsilon=0.01, is_train=False):
    ...
    if smoothing == 'none':
        ...
        if is_train:
            label_NN = netG(label)
            label_CG = CG(label_NN, tol=opt.CGtol, L=opt.Lambda, smap=smap, mask=mask, alised_image=input)
            label_CG_i = label_CG.repeat(num_sample, 1, 1, 1)
...
for epoch in tqdm(range(opt.epoch)):
    ...
    for direct, target, smap, mask in train_loader:
        ...
        output, loss_G = Recon(opt.blockIter, smap, mask, clean_input, label, opt.smoothing, opt.num_sample, opt.smoothing_epsilon, is_train=True)
    ...
    for vali_direct, vali_target, vali_smap, vali_mask in test_loader:
        ...
        with torch.no_grad():
            vali_result, _ = Recon(opt.blockIter, vali_smap, vali_mask, clean_vali_input, vali_label, opt.smoothing, opt.num_sample, opt.smoothing_epsilon)
