...
test_psnr = []
test_ssim = []
noise_test_psnr = [[] for _ in range(len(epsilons))]
noise_test_ssim = [[] for _ in range(len(epsilons))]
adv_test_psnr = [[] for _ in range(len(epsilons))]
adv_test_ssim = [[] for _ in range(len(epsilons))]
...
for i, (test_direct, test_target, test_smap, test_mask) in enumerate(test_loader):
    ...
    for ii, epsilon in enumerate(epsilons):
        noise_test_input = test_input.clone()
        noise_test_input = noise_test_input + torch.normal(0, epsilon, noise_test_input.shape).to(device)
        with torch.no_grad():
            noise_test_result = Recon(opt.blockIter, test_smap, test_mask, noise_test_input, opt.smoothing, opt.num_sample, opt.smoothing_epsilon)
            noise_psnr_test = PSNR(test_label, noise_test_result)
            noise_ssim_test = ssim_loss(test_label, noise_test_result)
            noise_test_psnr[ii].append(float(noise_psnr_test))
            noise_test_ssim[ii].append(float(noise_ssim_test))
            if vis and ii == 3:
                noise_vis_img = noise_test_result
    ...
    for ii, epsilon in enumerate(epsilons):
        ...
        with torch.no_grad():
            ...
            adv_test_psnr[ii].append(float(adv_psnr_test))
            adv_test_ssim[ii].append(float(adv_ssim_test))
            if vis and ii == 3:
                adv_vis_img = adv_test_result
    
    if vis:
        img_real = test_label.detach().cpu().numpy()
        img_real = img_real.squeeze(0)
        img_real = img_real[0] + img_real[1] * 1j

        img_out = test_result.detach().cpu().numpy()
        img_out = img_out.squeeze(0)
        img_out = img_out[0] + img_out[1] * 1j

        noise_vis_img = noise_vis_img.detach().cpu().numpy()
        noise_vis_img = noise_vis_img.squeeze(0)
        noise_vis_img = noise_vis_img[0] + noise_vis_img[1] * 1j
        
        adv_vis_img = adv_vis_img.detach().cpu().numpy()
        adv_vis_img = adv_vis_img.squeeze(0)
        adv_vis_img = adv_vis_img[0] + adv_vis_img[1] * 1j

        plt.figure(figsize=(4, 1))

        plt.subplot(1, 4, 1)
        plt.imshow(np.abs(img_real), cmap='gray')
        plt.title('original', fontsize='x-small')
        plt.axis('off')

        plt.subplot(1, 4, 2)
        plt.imshow(np.abs(img_out), cmap='gray')
        plt.title('recon', fontsize='x-small')
        plt.axis('off')

        plt.subplot(1, 4, 3)
        plt.imshow(np.abs(noise_vis_img), cmap='gray')
        plt.title('noise', fontsize='x-small')
        plt.axis('off')

        plt.subplot(1, 4, 3)
        plt.imshow(np.abs(adv_vis_img), cmap='gray')
        plt.title('robust', fontsize='x-small')
        plt.axis('off')

        plt.tight_layout()
        img_name = opt.netGpath[:-4] + '_test.pdf'
...
for ii in range(len(epsilons)):
    ...
    message += f'Noise PSNR: {np.average(noise_test_psnr[ii]):.4f} ± {np.std(noise_test_psnr[ii]):.4f}\n'
    message += f'Noise SSIM: {np.average(noise_test_ssim[ii]):.4f} ± {np.std(noise_test_ssim[ii]):.4f}\n'
message += '\n'
for ii in range(len(epsilons)):
    message += f'epsilon = {epsilons[ii] * 255:.1f}/255\n'
    message += f'Robust PSNR: {np.average(adv_test_psnr[ii]):.4f} ± {np.std(adv_test_psnr[ii]):.4f}\n'
    message += f'Robust SSIM: {np.average(adv_test_ssim[ii]):.4f} ± {np.std(adv_test_ssim[ii]):.4f}\n'

file_name = opt.netGpath[:-4] + '_test.out'
